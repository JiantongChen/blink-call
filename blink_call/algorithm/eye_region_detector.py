...
#     Detects face landmarks and extracts a combined eye-region bounding box.
#     """

#     def __init__(self, configs):
#         self.ctx_id = configs.get('ctx_id', -1)   # 默认 CPU
#         self.det_size = tuple(configs.get('det_size', (640, 640)))
#         self.det_thresh = configs.get('det_thresh', 0.5)
#         self.eye_padding = int(configs.get('eye_padding', 20))
#         self.max_num_faces = int(configs.get('max_num_faces', 0))
...
#         If visualization shows left/right swapped, exchange them here.
#         """
...
#     def _extract_eye_bbox(self, landmarks, frame_shape):
...
#         x_min = int(np.floor(np.min(all_eye_points[:, 0]))) - self.eye_padding
#         y_min = int(np.floor(np.min(all_eye_points[:, 1]))) - self.eye_padding
#         x_max = int(np.ceil(np.max(all_eye_points[:, 0]))) + self.eye_padding
#         y_max = int(np.ceil(np.max(all_eye_points[:, 1]))) + self.eye_padding
...
#         bbox = self._extract_eye_bbox(landmarks, frame.shape)

#         return {
#             "timestamp_ms": timestamp_ms,
#             "bbox_xyxy": bbox,
#             "debug_info": f"ok_landmark106_face_count_{len(faces)}"
#         }
